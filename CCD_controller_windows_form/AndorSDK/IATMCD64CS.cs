...
namespace ATMCD64CS {
    ...
    public interface IATMCD64CS
    {
        ...
        uint EnableSensorCompensation(int mode);
        ...
        uint GetBitsPerPixel(int readout_index, int index, ref int value);
        uint GetCameraEventStatus(ref int cam_status);
        uint GetESDEventStatus(ref int cam_status);
        ...
        unsafe uint GetCurrentPreAmpGain(ref int index, ref String name, int length);
        ...
        uint GetIRIGData(byte[] data, uint index);
        ...
        uint GetUSBDeviceDetails(ref ushort VendorID, ref ushort ProductID, ref ushort FirmwareVersion, ref ushort SpecificationNumber);
        ...
        uint SaveAsCalibratedSif(string path, int x_data_type, int x_unit, float[] x_cal, float rayleighWavelength);
        ...
        uint SetBitsPerPixel(int state);
        ...
        uint SetESDEvent(IntPtr driverevent);        
        ...
        uint SetFastKineticsStorageMode(int mode);
        uint SetFastKineticsTimeScanMode(int rows, int tracks, int mode);        
        ...
        uint SetIRIGModulation(char mode);
        ...
        uint SetSensorPortMode(int mode);
        uint SelectSensorPort(int port);
        uint SelectDualSensorPort(int port);        
