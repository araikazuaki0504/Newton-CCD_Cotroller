    ...
    public partial class mainForm : Form
    {
        ...
        private async void btton_Acuire_Frame_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                errorValue = AndorSDK.SetAcquisitionMode(AcquisitionMode.Runt_Till_Abort);
                if (errorValue != AndorSDK.DRV_SUCCESS)
                {
                ...
                else
                {
                    ...
                    //}
                    //
                    //errorValue = AndorSDK.GetAcquisitionTimings(ref exposure_time, ref accumulate_time, ref kinetic_time);
                    //if (errorValue != AndorSDK.DRV_SUCCESS)
                    //{
                    //    MessageBox.Show("測定時間を取得できませんでした。", "ERROR!", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    //    Temperature_timer.Enabled = true;
                    //    Temperature_timer.Start();
                    //    bttn_Acquire_Frame.Enabled = true;
                    //    bttn_Stop.Enabled = false;
                    ...
                    //}
                    //
                    //float TimeOut = exposure_time + accumulate_time + kinetic_time + shutterdaly;

                    EventWaitHandle Eventhandle = new EventWaitHandle(false, EventResetMode.ManualReset);

                    IntPtr handle =  Eventhandle.SafeWaitHandle.DangerousGetHandle();

                    errorValue = AndorSDK.SetDriverEvent(handle);
                    if (errorValue != AndorSDK.DRV_SUCCESS)
                    {
                        MessageBox.Show("測定を開始できませんでした。", "ERROR!", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        Temperature_timer.Enabled = true;
                        Temperature_timer.Start();
                        bttn_Acquire_Frame.Enabled = true;
                        bttn_Stop.Enabled = false;
